...
$updated_by = ($last_update && isset($last_update['full_name'])) ? $last_update['full_name'] : ($_SESSION['user_name'] ?? 'System');
$last_update_time = ($last_update && isset($last_update['last_update']) && !empty($last_update['last_update'])) ? date('h:i A', strtotime($last_update['last_update'])) : 'No updates today';
...
$top_items_query = "SELECT p.product_name, SUM(st.quantity) as total_used
                    ...
                    GROUP BY p.product_id ORDER BY total_used DESC LIMIT 4";
    ...
    <div class="print-stock-area">
        ...
        <div class="filter-section rounded-2xl p-6 mb-6">
            <form method="GET" action="" class="flex flex-wrap items-end gap-4">
                ...
                <div class="flex gap-2"><button type="submit" class="btn-apply text-white px-6 py-2 rounded-full text-sm font-bold uppercase tracking-wider">Apply Filters</button><a href="adminReports.php" class="btn-reset px-6 py-2 rounded-full text-sm font-bold uppercase tracking-wider">Reset</a></div>
            </form>
        </div>
...
function changeDailyDate() { 
    ...
    window.location.href = 'adminReports.php?daily_date=' + date; 
}
